...
if(config.payments.priority == null) {
    config.payments.priority = 0; // Default priority
}
        ...
        function(balances, callback){
            ...
            for (var worker in payments){
                ...
                if((addr.length == 1 && worker.length == 106) || addr.length == 2) {
                    paymentAmount -= config.payments.transferFee;
                    if(paymentAmount < 0) {
                        continue;
                    }
                    if(addresses > 0) {
                ...
                transferCommands[commandIndex].rpc.destinations.push({amount: paymentAmount, address: addr[0]});
                transferCommands[commandIndex].redis.push(['hincrby', config.coin + ':workers:' + worker, 'balance', -amount]);
                transferCommands[commandIndex].redis.push(['hincrby', config.coin + ':workers:' + worker, 'paid', paymentAmount]);
                transferCommands[commandIndex].amount += paymentAmount;
                if(with_payment_id) {
                    if(addr.length == 2) {
            ...
            async.filter(transferCommands, function(transferCmd, cback){
                apiInterfaces.rpcWallet('transfer', transferCmd.rpc, function(error, result){
                    ...
                    for (var i = 0; i < transferCmd.rpc.destinations.length; i++){
                        ...
                        if(!(transferCmd.rpc.payment_id == null)) {
